unit Unit_UBER_TH3CellIndex_version_005;

{

  UBER H3CellIndex implementation
...
type
	...
	private
		...
		procedure SetCell(ParaResolution: integer;
		  const Value: integer);
	...
	public
		...
		property Cell[ParaIndex: integer]: integer read GetCell
		  write SetCell;
...
const
	...
	ConstDigitMask = 1 + 2 + 4; // 7

	ConstBaseCellBitOffset = 45;
	ConstBaseCellMask = 1 + 2 + 4 + 8 + 16 + 32 + 64; // 127

	ConstModeBitOffset = 59;
	ConstModeMask = 1 + 2 + 4 + 8; // 15

	ConstResolutionBitOffset = 52;
	ConstResolutionMask = 1 + 2 + 4 + 8; // 15;

	ConstReservedBitOffset = 63;
	ConstReservedMask = 1;

function TH3CellIndex.GetCell(ParaResolution: integer): integer;
var
	vDigitIndex: integer;
	vShiftIndex: integer;
	vShiftedData: int64;
...
procedure TH3CellIndex.SetCell(ParaResolution: integer; const Value: integer);
var
	vDigitIndex: integer;
	vShiftIndex: integer;
...
(*
  function TH3CellIndex.GetBaseCell: integer;
  begin
  // shift down the data
  result := mData shr ConstBaseCellBitOffset;
  // isolate the field we want
  ...
  procedure TH3CellIndex.SetBaseCell(const Value: integer);
  var
  vMask : int64;
  vShiftedMask : int64;
  vInvertedMask : int64;
...
function TH3CellIndex.GetMode: integer;
begin
	// shift down the data
...
procedure TH3CellIndex.SetMode(const Value: integer);
var
	vMask: int64;
	vShiftedMask: int64;
	vInvertedMask: int64;
...
function TH3CellIndex.GetResolution: integer;
begin
	// shift down the data
...
procedure TH3CellIndex.SetResolution(const Value: integer);
var
	vMask: int64;
	vShiftedMask: int64;
	vInvertedMask: int64;
...
function TH3CellIndex.GetReserved: integer;
begin
	// shift down the data
...
procedure TH3CellIndex.SetReserved(const Value: integer);
var
	vMask: int64;
	vShiftedMask: int64;
	vInvertedMask: int64;
